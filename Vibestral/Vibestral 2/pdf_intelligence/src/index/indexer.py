# src/index/indexer.py
# Changes: Replaced manual BEGIN/COMMIT/ROLLBACK with 'with conn:' context manager
# (correct for isolation_level=None WAL mode); fixed mojibake; kept Lovable's
# INTEGER PK and corpus_stats() at chunk level for correct BM25 avg_dl.
"""Indexer: ingest PDF/EPUB -> SQLite + FTS5 + BM25 stats.

Incremental reindex: skip a file if (file_size, file_mtime) match stored fingerprint.
Otherwise delete its rows and re-ingest. Avoids hashing GB of PDFs on every scan.
"""
...
def index_file(conn: sqlite3.Connection, path: Path, *, ocr: bool = False) -> str:
    ...
    # 'with conn:' on isolation_level=None issues BEGIN/COMMIT/ROLLBACK correctly
    with conn:
        cur = conn.execute(
        ...
        for idx, ch in enumerate(chunks):
            tf_rows = _insert_chunk(conn, doc_id, idx, ch)
            term_freq_batch.extend(tf_rows)
...
def _insert_chunk(conn: sqlite3.Connection, doc_id: int, idx: int, ch: Chunk) -> None:
    ...
    if stems:
        ...
        return [(ch.id, term, tf) for term, tf in counts.items()]
    return []
...
def corpus_stats(conn: sqlite3.Connection) -> tuple[int, float]:
    """Returns (n_chunks, avg_chunk_tokens) -- used by BM25 at chunk level."""
    row = conn.execute(
